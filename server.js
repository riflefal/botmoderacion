...
client.on("guildMemberAdd", async member => {
  ...
  client.channels.cache.get(chx).send("Bienvenido al servidor " + member.user.username, attachment);
...
fetch(`https://api.affiliateplus.xyz/api/chatbot?message=${encodeURIComponent(message.content)}&botname=${client.user.username}&ownername=verde#4726`)
    .then(res => res.json())
    .then(data => {
...
client.on("ready", () => {
    ...
    console.log("automodv12 beta is ready join support server https://dsc.gg/pipobot")
});
...
client.on
client.on("ready", () => {
    client.user.setActivity(`s!help para ayuda |Servers Count - ${client.guilds.cache.size}`, { type: "WATCHING"})
})
...
client.on("message", (message) => {
    if (message.content.startsWith("!creador")) {
        message.channel.send("Mi creador es fall#0215");
    }
